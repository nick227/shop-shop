...
import {
  useStores as useStoresRaw,
  useStore as useStoreRaw,
  useOrders as useOrdersRaw,
} from './generated'
...
} from '@features/search/utils/searchOptimizations'
...
export function useStores(location: LocationData | undefined): UseStoresResult {
  ...
  // Validate coordinates to prevent bad API calls
  ...
  const {
    ...
    error: rawError,
    refetch,
  } = useStoresRaw(locationParams, {
    enabled: Boolean(hasValidLocation),
  })
  ...
  const stores = useMemo(() => {
    ...
    const searchResults = rawStores.map(
