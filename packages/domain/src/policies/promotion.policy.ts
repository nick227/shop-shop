...
export class PromotionPolicy implements BasePolicy<Promotion> {
  /**
   * Authorization: Can user create this promotion?
   */
  async canCreate(data: unknown, context: PolicyContext): Promise<boolean | string> {
    const input = data as { storeId?: string }
   ...
   */
  async canUpdate(promotion: Promotion, context: PolicyContext): Promise<boolean | string> {
   ...
   */
  async canDelete(promotion: Promotion, context: PolicyContext): Promise<boolean | string> {
