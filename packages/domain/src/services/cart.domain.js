...
export class CartDomain {
     ...
     */
    async ensureActiveCart(userId, storeId) {
     ...
     */
    async addItemToCart(userId, itemId, quantity, optionsJson, notes) {
        // Get item details
        ...
        if (existingCartItem) {
            ...
            cartItem = await prisma.cartItem.update({
                ...
                data: {
                    ...
                    optionsJson: optionsJson, // Prisma JSON type
                    notes,
                },
            });
        }
     ...
     */
    async addBundleToCart(userId, bundleId, notes) {
        ...
        if (unavailable.length > 0)
            throw new Error('One or more items in this bundle are unavailable');
     ...
     */
    async updateCartItem(cartItemId, userId, quantity, optionsJson, notes) {
        // Verify ownership
        ...
        if (quantity === 0) {
            ...
            await prisma.cartItem.update({
                ...
                data: {
                    quantity,
                    ...(optionsJson && { optionsJson: optionsJson }), // Prisma JSON type
    ...
    async validateCartForCheckout(cartId) {
        ...
        for (const ci of cart.items) {
            if (ci.bundleId) {
                if (!ci.bundle)
                    return { valid: false, reason: `Bundle "${ci.titleSnapshot}" no longer exists` };
