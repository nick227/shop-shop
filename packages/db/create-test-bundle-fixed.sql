...
-- Get existing items from the store
SELECT @item1 := id FROM Item WHERE storeId = @storeId LIMIT 1;
SELECT @item2 := id FROM Item WHERE storeId = @storeId LIMIT 1 OFFSET 1;
SELECT @item3 := id FROM Item WHERE storeId = @storeId LIMIT 1 OFFSET 2;
