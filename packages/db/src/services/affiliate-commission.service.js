...
// ─── Internal helpers ─────────────────────────────────────────────────────────
async function loadAffiliateSettings(db) {
    const keys = [
 ...
 */
export function resolveRateFromValues(overrideBps, groupBps, systemDefaultBps, payoutGroupId) {
    if (overrideBps !== null) {
        return {
            rateBps: overrideBps,
...
export function calculateCommissionAmountCents(commissionBaseCents, rateBps) {
 ...
 */
export function applyDualCommissionCap(candidates, serviceFeeAmountCents, maxBurdenBps) {
    ...
    const sorted = [...candidates].sort((a, b) => (priority[a.sourceType] ?? 99) - (priority[b.sourceType] ?? 99));
    let burdenUsed = 0;
    return sorted.map((c) => {
 ...
 */
export async function resolveAffiliateRate(affiliateId, sourceType, db = prisma) {
 ...
 */
export async function buildAffiliateCommissionCandidatesForOrder(orderId, db = prisma) {
    ...
    if (!rawBuyerAffiliate && !rawStoreAffiliate)
        return [];
    const serviceFeeAmountCents = Math.round(Number(order.serviceFeeAmount) * 100);
    const settings = await loadAffiliateSettings(db);
    const candidates = [];
    // ── CUSTOMER_PURCHASE ──────────────────────────────────────────────────────
    const buyerAffiliate = rawBuyerAffiliate?.status === 'ACTIVE' &&
        rawBuyerAffiliate.userId !== order.userId // skip self-referral
        ? rawBuyerAffiliate
        : null;
    if (buyerAffiliate) {
        const { rateBps, rateSource, payoutGroupIdSnapshot } = await resolveAffiliateRate(buyerAffiliate.id, 'CUSTOMER_PURCHASE', db);
        candidates.push({
    ...
    const storeAffiliate = rawStoreAffiliate?.status === 'ACTIVE' &&
        !isSameAsBuyer &&
        rawStoreAffiliate.userId !== store.ownerUserId // skip self-referral
        ? rawStoreAffiliate
        : null;
    if (storeAffiliate) {
        const { rateBps, rateSource, payoutGroupIdSnapshot } = await resolveAffiliateRate(storeAffiliate.id, 'STORE_REVENUE', db);
        candidates.push({
 ...
 */
export async function upsertCommissionCandidate(candidate, db = prisma) {
