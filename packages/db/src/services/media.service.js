...
import { getStorageAdapter, validateMediaFile } from '../adapters/storage.adapter.js';
...
export const deleteMedia = async (input) => {
    ...
    if (storeId || itemId) {
        ...
        await prisma.$transaction(remainingMedia.map((asset, index) => prisma.mediaAsset.update({
            where: { id: asset.id },
...
export const updateMediaSort = async (input) => {
    const media = await prisma.mediaAsset.findUnique({
        where: { id: input.mediaId },
        include: {
            store: true,
...
export const reorderMedia = async (input) => {
    ...
    await prisma.$transaction(input.mediaIds.map((mediaId, index) => prisma.mediaAsset.update({
        where: { id: mediaId },
        data: { sortIndex: index }
    })));
};
