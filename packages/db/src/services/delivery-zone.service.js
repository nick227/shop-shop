...
export async function updateDeliveryZone(zoneId, input) {
    const data = {};
    if (input.name !== undefined)
        data.name = input.name;
    if (input.baseFee !== undefined)
        data.baseFee = input.baseFee;
    if (input.minOrder !== undefined)
        data.minOrder = input.minOrder;
    if (input.isActive !== undefined)
        data.isActive = input.isActive;
    if (input.priority !== undefined)
        data.priority = input.priority;
    if (input.polygonJson !== undefined)
        data.polygonJson = input.polygonJson;
...
export async function getStoreDeliveryZones(storeId) {
    return prisma.deliveryZone.findMany({
        where: { storeId },
        orderBy: [{ priority: 'desc' }, { createdAt: 'asc' }],
    });
}
export async function getActiveStoreDeliveryZones(storeId) {
    return prisma.deliveryZone.findMany({
        where: {
            storeId,
            isActive: true,
...
export async function findDeliveryZoneForAddress(storeId, coordinates) {
    const zones = await getActiveStoreDeliveryZones(storeId);
    for (const zone of zones) {
        const polygonData = zone.polygonJson;
...
export async function calculateDeliveryFee(storeId, coordinates, orderSubtotal) {
...
export async function bulkUpdateZonePriorities(updates) {
    await Promise.all(updates.map((update) => prisma.deliveryZone.update({
