...
export const checkStoreActivationRequirements = async (storeId) => {
    ...
    const hasRequiredFields = !!(store.name &&
        store.description &&
    ...
    if (activeItemsCount === 0) {
        ...
        hasActiveBundles = activeBundles.some((b) => b.items.length > 0 &&
...
export const checkProductActivationRequirements = async (productId) => {
    ...
    // Check required fields
    const hasRequiredFields = !!(product.title &&
        product.description &&
        product.price &&
        product.price.gt(0));
    // Check media requirements
...
export const checkBundleActivationRequirements = async (bundleId) => {
    ...
    const hasActiveItems = hasMinimumItems &&
        allItemsFromSameStore &&
        bundle.items.every((bi) => bi.item.isActive && !bi.item.isSoldOut);
    const { isActive } = bundle;
    const { canAppearInMarketplace: parentStorePublic } = await checkStoreActivationRequirements(bundle.storeId);
