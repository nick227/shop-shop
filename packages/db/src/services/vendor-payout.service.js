...
import { computePayoutBreakdownCents, decimalToCents, isInUtcPeriodInclusiveExclusive, isPayableOrder, snapshotOrderForPayout, } from './payout-calculation.service.js';
/**
 * Get payout summary for a store for a given period
 */
export async function getVendorPayoutSummary(storeId, periodStart, periodEnd) {
 ...
 */
export async function processVendorPayout(input) {
    const store = await prisma.store.findUnique({
    ...
    const payableSnapshots = orders
        .filter((o) => isPayableOrder({ status: o.status, paymentStatus: o.paymentStatus }))
        .filter((o) => isInUtcPeriodInclusiveExclusive({ at: o.createdAt, periodStart: input.periodStart, periodEnd: input.periodEnd }))
        .map(snapshotOrderForPayout);
 ...
 */
export async function getStoresReadyForPayout(periodStart, periodEnd) {
 ...
 */
export async function processAllVendorPayouts(periodStart, periodEnd) {
    const eligible = await getStoresReadyForPayout(periodStart, periodEnd);
    const results = await Promise.allSettled(eligible.map((summary) => processVendorPayout({
 ...
 */
export async function getVendorPayoutHistory(storeId, options) {
 ...
 */
export async function getPendingPayoutAmount(storeId) {
