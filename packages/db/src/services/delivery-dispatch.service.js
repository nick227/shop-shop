...
async function assertActiveStoreDriver(userId, storeId) {
    const member = await prisma.teamMember.findFirst({
        where: { storeId, userId, isActive: true },
        select: { permissionsJson: true },
    });
    const permissions = Array.isArray(member?.permissionsJson)
        ? member.permissionsJson.filter((p) => typeof p === 'string')
        : [];
    const ok = permissions.includes('FULL_ACCESS') ||
...
function mergeProviderPayloadForCancel(existing, cancelPayload, reason) {
 ...
 */
export async function cancelDeliveryJob(input) {
...
export async function dispatchOrderDelivery(input) {
    ...
    if (order.status !== 'READY')
        throw new Error('Order must be READY to dispatch');
 ...
 */
export async function applyDeliveryProviderWebhookEvent(input) {
    const job = await prisma.deliveryJob.findUnique({
        where: { id: input.deliveryJobId },
    ...
    const nextJobStatus = mapped.providerStatus === 'delivered'
        ? 'COMPLETED'
    ...
    const updatedJob = await prisma.deliveryJob.update({
        ...
        data: {
            ...
            providerPayload: mapped.providerPayload,
            ...(nextJobStatus ? {
