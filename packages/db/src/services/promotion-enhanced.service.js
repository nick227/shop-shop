...
export async function validatePromotionCode(input) {
    ...
    const eligibleUserIds = promotion.eligibleUserIds;
    ...
    const excludedUserIds = promotion.excludedUserIds;
 ...
 */
export async function getUserPromotionHistory(userId) {
 ...
 */
export async function canUserUsePromotion(promotionId, userId) {
    const promotion = await prisma.promotion.findUnique({
        where: { id: promotionId },
    });
    if (!promotion) {
 ...
 */
export async function getActivePromotionsForStore(storeId) {
    const now = new Date();
