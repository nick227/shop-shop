 ...
 */
async function* streamToCSV(dataGenerator, headers, transform) {
    // Yield headers first
    yield headers.join(',') + '\n';
    // Stream data rows
 ...
 */
async function* batchFetchOrders(where, batchSize = 1000) {
    let cursor;
    while (true) {
 ...
 */
export async function exportCommissionsToCSV(options = {}) {
    const where = {};
 ...
 */
export async function exportPayoutsToCSV(options = {}) {
    const where = {};
    if (options.status) {
        where.status = options.status;
    }
    if (options.startDate || options.endDate) {
        where.periodEnd = {};
        if (options.startDate) {
            where.periodEnd.gte = options.startDate;
        }
        if (options.endDate) {
            where.periodEnd.lte = options.endDate;
        }
    }
 ...
 */
export async function exportOrdersToCSV(options = {}) {
    const where = {};
    if (options.storeId) {
 ...
 */
export async function* streamOrdersToCSV(options = {}) {
    const where = {};
    if (options.storeId) {
        where.storeId = options.storeId;
    }
    ...
    for await (const orders of batchFetchOrders(where)) {
        for (const order of orders) {
            yield* streamToCSV((async function* () { yield order; })(), headers, transform);
        }
    }
}
/**
 ...
 */
export async function exportTaxSummaryToCSV(options = {}) {
    const where = {};
    if (options.storeId) {
        where.storeId = options.storeId;
    }
 ...
 */
export async function exportServiceFeesToCSV(startDate, endDate) {
    const where = {};
 ...
 */
export async function exportFinancialSummaryToCSV(startDate, endDate) {
    const where = {};
    if (startDate || endDate) {
        where.createdAt = {};
        if (startDate) {
 ...
 */
export async function exportVendorPayoutsToCSV(startDate, endDate) {
    const where = {};
    if (startDate || endDate) {
        where.createdAt = {};
        if (startDate) {
