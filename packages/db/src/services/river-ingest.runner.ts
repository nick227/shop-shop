...
import {
  firstItemImageMedia,
  firstStoreImageMedia,
  loadAutomationKeys,
  riverIngestKey,
} from './river-ingest.helpers.js'
import type { RiverIngestResult } from './river-ingest.types.js'

const emptyResult = (): RiverIngestResult => ({
  created: 0,
  skippedExisting: 0,
  skippedNoMedia: 0,
  skippedCooldown: 0,
...
function mergeResult(
  a: RiverIngestResult,
  b: Partial<Record<keyof RiverIngestResult, number>>,
): RiverIngestResult {
  return {
 ...
 */
export async function runRiverIngestion(
  db: ExtendedPrismaClient,
  options?: Readonly<{ enableRestock?: boolean }>,
