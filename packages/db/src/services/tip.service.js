...
import { createPaymentIntent as stripeCreatePaymentIntent, createRefund as stripeCreateRefund, } from '../adapters/payments.adapter.js';
// WebSocket notification function (set by server)
let broadcastTipEvent = null;
export const setTipServiceBroadcast = (fn) => {
    broadcastTipEvent = fn;
};
export const createTip = async (input) => {
...
export const processTip = async (input) => {
    ...
    try {
        ...
        }; // Metadata not in type definition but may be needed
        ...
        const updatedTip = await prisma.tip.update({
            ...
            data: {
                ...
                stripeApplicationFeeId: paymentIntent.application_fee_amount ? 'fee_' + paymentIntent.id : null,
            },
        });
...
export const getTip = async (tipId, userId) => {
    const tip = await prisma.tip.findUnique({
        where: { id: tipId },
        include: {
            order: true,
...
export const refundTip = async (tipId, userId) => {
    const tip = await prisma.tip.findUnique({
        where: { id: tipId },
        include: {
            order: true,
