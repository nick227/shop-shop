 ...
 */
export async function computeBundlePrice(bundleId) {
    ...
    const itemSum = bundle.items.reduce((acc, bi) => acc.plus(D(bi.item.price.toString()).times(bi.quantity)), new Decimal(0));
    if (!bundle.pricing) {
    ...
    if (p.fixedPrice != null)
        candidates.push(D(p.fixedPrice.toString()));
    ...
    if (p.discountAmount != null)
        candidates.push(itemSum.minus(D(p.discountAmount.toString())));
    let resolvedPrice;
    ...
    if (resolvedPrice.lessThan(0))
        resolvedPrice = new Decimal(0);
 ...
 */
export async function snapshotBundle(bundleId) {
    const bundle = await prisma.bundle.findUnique({
        where: { id: bundleId },
        include: {
            pricing: true,
