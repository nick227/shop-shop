...
export const CreateStoreInputSchema = generateCreateInputSchema({
  ...
  overrides: {
    ...
    deliveryDistance: z.string().regex(/^\d+(\.\d{1,2})?$/).refine(
      val => parseFloat(val) >= 0 && parseFloat(val) <= 100,
      'Delivery distance must be between 0 and 100 miles'
    ).optional(),
    deliveryCharge: z.string().regex(/^\d+(\.\d{1,2})?$/).refine(
...
export const UpdateStoreInputSchema = generateUpdateInputSchema({
  ...
  overrides: {
    ...
    email: z.string().email().optional(),
    website: z.string().url().optional(),
    commissionRate: z.string().regex(/^\d+(\.\d{1,2})?$/).refine(
      val => parseFloat(val) >= 0 && parseFloat(val) <= 100,
