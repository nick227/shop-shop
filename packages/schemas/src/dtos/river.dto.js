...
export const RiverFeedQuerySchema = z
    ...
    const hasGeo = latNum !== undefined &&
        lngNum !== undefined &&
        Number.isFinite(latNum) &&
        Number.isFinite(lngNum);
    const rm = Number(data.radiusMiles ?? '');
    const radiusMiles = hasGeo && Number.isFinite(rm) && rm > 0 ? rm : hasGeo ? 25 : undefined;
    return {
        cursor: data.cursor,
        limit: Math.min(Math.max(limit, 1), 50),
        storeId: data.storeId,
        near: hasGeo && latNum !== undefined && lngNum !== undefined && radiusMiles !== undefined
